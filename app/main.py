...
async def api_restart_service(
    ...
    if not ENABLE_SERVICE_RESTART:
        raise HTTPException(status_code=404, detail="SERVICE_RESTART_DISABLED")
    ...
    if sys.platform.startswith("win"):
        return {"ok": False, "error": "NOT_SUPPORTED_ON_WINDOWS"}

    cmd = TRADING_RESTART_CMD

    try:
        # Use a subprocess so we can return quickly (important if restarting a related service).
        p = subprocess.Popen(
            cmd,
            shell=True,
    ...
    # Best-effort: try to read a small amount of output without blocking long.
    try:
        out, err = p.communicate(timeout=3)
        rc = int(p.returncode or 0)
        if rc != 0:
            return {"ok": False, "error": f"RESTART_FAILED:{rc}", "stderr": (err or "").strip(), "stdout": (out or "").strip()}
        return {"ok": True, "message": "Restart requested", "stdout": (out or "").strip()}
    except Exception:
        # Likely still running; that's fine.
...
# ---------------------------------------------------------------------------
# NOTE: The remainder of this file was accidentally duplicated (paste/merge).
# It is disabled to prevent duplicate routes and multiple startup handlers.
# ---------------------------------------------------------------------------
'''
# -----------------------------
def _read_dashboard_template(user_id: int, username: str) -> str:
